...
export default class Game extends Component {
  constructor(props) {
    ...
    this.successReset = () => {
      Matter.Body.setPosition(this.entities.puck.body, {
        x: Constants.MAX_WIDTH / 2,
        y: Constants.MAX_HEIGHT / 2,
      });
      Matter.Body.setPosition(this.entities.player1.body, {
        x: Constants.MAX_WIDTH / 2,
        y: 210,
      });
    };
    ...
    this.resume = () => {
      ...
      Matter.Body.setVelocity(this.entities.puck.body, {
        x: Math.floor(Math.random() * (20 - 10 + 1) + 10),
        y: Math.floor(Math.random() * (20 - 10 + 1) + 10),
      });
    };
  }
  ...
  setupWorld = () => {
    ...
    let wallLeft = Matter.Bodies.rectangle(-95, height / 2, 200, height - 100, {
      isStatic: true,
    });
    ...
    let top_boundary = Matter.Bodies.rectangle(width / 2, -10, 10000, 20, {
    ...
    let puck = Matter.Bodies.circle(width / 2, height / 2, 20, {
      inertia: 0,
      friction: 0,
      frictionStatic: 0,
      frictionAir: 0.0,
    ...
    let player1 = Matter.Bodies.circle(width / 2, 100, 45, {label: 'player1'});
    let player2 = Matter.Bodies.circle(width / 2, height - 100, 45, {
      label: 'player2',
    });

    let topLeftWall = Matter.Bodies.rectangle(-50, 50, 40, height / 2, {
      isStatic: true,
      label: 'successBoundary',
    });

    let topRightWall = Matter.Bodies.rectangle(width + 50, 50, 20, height / 2, {
      isStatic: true,
      label: 'successBoundary',
    });
    ...
    let bottomLeftWall = Matter.Bodies.rectangle(
      ...
      20,
      height / 2,
      {isStatic: true, label: 'failBoundary'},
    );
    ...
    Matter.Events.on(engine, 'collisionStart', (event) => {
      ...
      if (
        (objA === 'puck' && (objB === 'player1' || objB === 'player2')) ||
        ((objA === 'player1' || objA === 'player2') && objB === 'puck')
      ) {
      ...
      if (
        (objA === 'successBoundary' && objB === 'puck') ||
        (objB === 'succesBoundary' && objA === 'puck')
      ) {
      ...
      if (
        (objA === 'failBoundary' && objB === 'puck') ||
        (objB === 'failBoundary' && objA === 'puck')
      ) {
        if (this.state.running)
          this.setState({
    ...
    return {
      ...
      bottom_goal_left: {
        body: bottom_goal_left,
        size: [(width - gap_size) / 2, 5],
        color: 'white',
        renderer: Wall,
      },
      bottom_goal_right: {
        body: bottom_goal_right,
        size: [(width - gap_size) / 2, 5],
        color: 'white',
        renderer: Wall,
      },
  ...
  render() {
    return (
      <SafeAreaView style={styles.container}>
        ...
        {this.state.paused && !this.state.end && !this.state.running && (
          <TouchableOpacity
        ...
        {!this.state.paused && !this.state.running && this.state.welcome && (
          <TouchableOpacity
            style={styles.fullScreenButton}
            onPress={this.reset}>
            <View style={styles.fullScreen}>
        ...
        {this.state.end && this.state.welcome && (
          <TouchableOpacity
            style={styles.fullScreenButton}
            onPress={this.reset}>
            <View style={styles.fullScreen}>
